...
exports.GetGroupedCountries = async function (res) {
  try {
    let continents = await mongoose.retrieve(model.Continent, {}); // Henter continents.
    //let continents = await mongoose.retrieveGroupedCountryDistinct("continent"); // Henter alle continents...
    let countries = await mongoose.retrieve(model.Country); // Alle countries distinct.

    let sortedContinents = await mongoose.retrieveAndSort(model.Continent, {
      sort: { continent: -1 }, // sorteret :-)
    });
    /*
        continents.forEach(continent => {
            listOfContinents.push({continent});
        });*/
    function BuildContinent(name) {
      this.name = name;
      this.countries = [];
    }

    // Opbygning af nested objekter
    /*
        Buildcontinent {
            name: "Europe",
            countries : [
    ...
    */
    ...
    for (let i = 0; i < sortedContinents.length; i++) {
      // console.log(continents[i].name); /// Henter alle navne på continents
      continentsWithCountries.push(
        new BuildContinent(sortedContinents[i].name)
      );
      for (let a = 0; a < countries.length; a++) {
        if (continentsWithCountries[i].name == countries[a].continent) {
          continentsWithCountries[i].countries.push(countries[a].name);
        }
      }
    }

    res.render('groupCountries', {
      title: 'grouped',
...
exports.searchRanking = async function (res, searchedLanguage) {
  try {
    ...
    let rank = await mon.retrieve(model.Countrylanguage, {
